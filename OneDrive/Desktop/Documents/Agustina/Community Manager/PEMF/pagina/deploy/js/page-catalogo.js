...
(function() {
  ...
  function openModal(prod, brand) {
    ...
    const ids = {
      ...
      mQA: document.getElementById('mQA'),
      mReviewsSummary: document.getElementById('mReviewsSummary'),
    ...
    if (ids.mTagline) ids.mTagline.textContent = prod.tagline || prod.desc;
    ...
    if (ids.mBenefits && prod.benefits) {
      ids.mBenefits.innerHTML = prod.benefits.map(b => `<div style="font-size:13px;color:#1a2332;margin-bottom:8px;display:flex;gap:8px;"><span style="color:#67CCE9;font-weight:800;">✓</span> ${b}</div>`).join('');
    }

    if (ids.mScience && prod.science) {
      ids.mScience.textContent = prod.science;
    }

    if (ids.mQA && prod.qa) {
      ids.mQA.innerHTML = prod.qa.map(item => `
        <div class="qa-item">
    ...
    if (ids.mReviewsSummary && prod.reviews) {
      const avgRating = Math.round(prod.reviews.reduce((sum, r) => sum + r.rating, 0) / prod.reviews.length * 10) / 10;
      const stars = '★'.repeat(Math.floor(avgRating)) + (avgRating % 1 >= 0.5 ? '☆' : '');
      ids.mReviewsSummary.innerHTML = `
        <div class="reviews-summary">
          <div class="reviews-avg">${avgRating}</div>
          <div>
            <div class="reviews-stars-big">${stars}</div>
            <div class="reviews-count">${prod.reviews.length} opiniones verificadas</div>
          </div>
        </div>
      `;
    }

    if (ids.mReviews && prod.reviews) {
      ids.mReviews.innerHTML = prod.reviews.map(review => `
        <div class="review-item">
          <div class="review-header">
            <div class="review-author">${review.author}</div>
            <div class="review-date">Compra verificada</div>
          </div>
          <div class="review-stars">${'★'.repeat(review.rating)}</div>
          <div class="review-text">${review.text}</div>
          ${review.verified ? '<div class="review-verified">✓ Verificado</div>' : ''}
        </div>
      `).join('');
    }
